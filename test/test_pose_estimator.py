...
def test_set_position_and_update_deltas():
    ...
    estimator.set_pose_and_update_deltas(position)
    position += delta
    estimator.set_pose_and_update_deltas(position)
    position -= 2*delta
    estimator.set_pose_and_update_deltas(position)
...
def test_is_paused():
    estimator = TestPoseEstimator()
    estimator.set_pose_and_update_deltas(np.array([1.0, 2, 3]))
    estimator.is_paused = True
    estimator.set_pose_and_update_deltas(np.array([1.0, 2, 4]))
    assert estimator.latest_deltas is None

    # the hand is moved to somewhere else
    estimator.set_pose_and_update_deltas(np.array([0.0, -1.0, 0.0]))
    # when pause is resumed, the deltas should be calculated from the new position.
    estimator.is_paused = False
    estimator.set_pose_and_update_deltas(np.array([1.0, -1.0, 0.0]))
    actual_delta = estimator.get_deltas()
